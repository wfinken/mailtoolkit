...
11. Cargo.lock pins the resolved dependency graph. CI produces platform CLI artifacts and a locally signed macOS app ZIP after tests pass. There is no SBOM, notarization or official signed release pipeline yet. Ubuntu 22.04/24.04 and macOS 14 arm64 pass the automated build and test matrix. See [validation details](VALIDATION.md). RHEL-family support is provisional; no retired CentOS release is promised.
