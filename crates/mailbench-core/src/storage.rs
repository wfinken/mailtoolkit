use crate::{smtp::SmtpOptions, Session};
...
pub struct Profile {
    ...
    pub sending_ip: Option<std::net::IpAddr>,
...
pub fn root() -> Result<PathBuf> {
    if let Some(path) = std::env::var_os("MAILBENCH_CONFIG_DIR") {
        return Ok(path.into());
    }
    if let Some(path) = std::env::var_os("XDG_CONFIG_HOME") {
        return Ok(PathBuf::from(path).join("mailbench"));
    }
    Ok(
        PathBuf::from(std::env::var_os("HOME").context("HOME is unset; set MAILBENCH_CONFIG_DIR")?)
            .join(".config/mailbench"),
    )
}
pub fn safe_name(name: &str) -> Result<()> {
    ...
    {
        bail!("Name must use only letters, numbers, hyphens, and underscores");
    }
    Ok(())
}
...
pub fn profiles() -> Result<Vec<Profile>> {
    ...
    for entry in fs::read_dir(path)? {
        ...
        if entry.path().extension().is_some_and(|s| s == "toml") {
            items.push(toml::from_str(&fs::read_to_string(entry.path())?)?);
        }
    }
    Ok(items)
}
...
pub fn sessions() -> Result<Vec<Session>> {
    let path = root()?.join("sessions");
    if !path.exists() {
        return Ok(vec![]);
    }
    let mut items: Vec<Session> = vec![];
    for entry in fs::read_dir(path)? {
        let entry = entry?;
        if entry.path().extension().is_some_and(|s| s == "json") {
            items.push(serde_json::from_slice(&fs::read(entry.path())?)?);
        }
    }
...
pub fn delete_session(id: &str) -> Result<()> {
    safe_name(id)?;
    fs::remove_file(root()?.join("sessions").join(format!("{id}.json")))?;
    Ok(())
}
pub fn parse_profile(text: &str) -> Result<Profile> {
    ...
    if profile.domain.is_empty() {
        bail!("Profile domain is empty");
    }
    if !(1..=300).contains(&profile.smtp.timeout_secs) {
        bail!("Profile timeout must be 1..300 seconds");
    }
    Ok(profile)
}
...
mod tests {
    ...
    fn blocks_path_escape() {
        for s in ["../x", "/tmp/a", "a/b", ""] {
            assert!(safe_name(s).is_err());
        }
        assert!(safe_name("customer-a_2").is_ok());
    }
}
