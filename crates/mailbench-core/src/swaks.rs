use crate::{
    smtp::{single_line, SmtpOptions, TlsMode},
    Finding, Status,
};
...
pub fn args(
    ...
    if o.source_ip.is_some() || o.sni.is_some() || o.ca_file.is_some() {
        bail!("Custom source IP, SNI and CA files require the native sender");
    }
    for value in [o.host.as_str(), o.ehlo.as_str(), sender, subject] {
        single_line(value)?;
    }
    for r in recipients {
        single_line(r)?;
    ...
    let mut args = vec![
        ...
        "--header".into(),
        format!("X-Mailbench-ID: {}", uuid::Uuid::new_v4()),
        "--body".into(),
        body.into(),
        "--timeout".into(),
        o.timeout_secs.to_string(),
    ];
    match o.tls {
        TlsMode::Starttls => args.push("--tls".into()),
        TlsMode::Implicit => args.push("--tls-on-connect".into()),
        TlsMode::Off => {}
    }
    if o.tls != TlsMode::Off && !o.no_verify {
        args.push("--tls-verify".into());
    }
    Ok(args)
}
...
pub async fn run(args: &[String], timeout: Duration) -> Finding {
    ...
    if args
        .iter()
        .any(|a| a.to_ascii_lowercase().starts_with("--auth") || a == "-ap" || a == "-au")
    {
        return Finding::error(
            "swaks",
            "swaks",
            "Use native send with --password-env or --password-stdin for authentication",
        );
    }
    let mut cmd = Command::new("swaks");
