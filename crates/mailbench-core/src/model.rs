...
impl Finding {
    ...
    pub fn advice(mut self, text: &str) -> Self {
...
impl Session {
    ...
    pub fn exit_code(&self) -> i32 {
        if self.findings.iter().any(|f| f.status == Status::Error) {
            4
        } else if self.findings.iter().any(|f| f.status == Status::Fail) {
            1
        } else if self.findings.iter().any(|f| f.status == Status::Warn) {
            2
        } else {
            0
        }
    ...
    pub fn markdown(&self) -> String {
        ...
        for f in &self.findings {
            ...
            for line in serde_json::to_string_pretty(&f.evidence)
                .unwrap_or_default()
                .lines()
            {
